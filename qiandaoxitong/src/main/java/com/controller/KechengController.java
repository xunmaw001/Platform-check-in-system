    ...
    */
    ...
    public R save(@RequestBody KechengEntity kecheng, HttpServletRequest request){
        ...
        if(kechengEntity==null){
            ...
            ;
            Wrapper<YonghuEntity> yonghuEntity = new EntityWrapper<YonghuEntity>()
                    .eq("banji_types", kecheng.getBanjiTypes());
            List<YonghuEntity> yonghulist = yonghuService.selectList(yonghuEntity);
            if(yonghulist.size() > 0){
                ArrayList<KechengkaoqinEntity> kechengkaoqinEntities = new ArrayList<>();
                for (YonghuEntity yh:yonghulist) {
                    KechengkaoqinEntity kechengkaoqinEntity = new KechengkaoqinEntity();
                    kechengkaoqinEntity.setInsertTime(new Date());
                    kechengkaoqinEntity.setCreateTime(new Date());
                    kechengkaoqinEntity.setYonghuId(yh.getId());
                    kechengkaoqinEntity.setKechengId(kecheng.getId());
                    kechengkaoqinEntity.setKechengkaoqinTypes(1);
                    kechengkaoqinEntities.add(kechengkaoqinEntity);
                }
                kechengkaoqinService.insertBatch(kechengkaoqinEntities);
            }
